...
class MetadataSyncService:
    ...
    async def sync_readme_to_repository(
    ...
    async def ensure_bidirectional_sync(
        self, repository: Repository, readme_content: Optional[str] = None
    ) -> str:
        """
        确保双向同步
        ...
        try:
            if readme_content:
                # README被更新，同步到数据库
                success = await self.sync_readme_to_repository(
    ...
    async def _build_yaml_from_repository(
        ...
        try:
            ...
            logger.warning(
                f"Failed to load sphere classifications for repository {repository.id}: {e}"
            )

        # 从关联分类获取task classifications
        try:
            task_classifications = await self._get_repository_task_classifications(
                repository.id
            )
            if task_classifications:
                yaml_data["tasks"] = task_classifications  # 使用 "tasks" 而不是 "task_classifications"
        except Exception as e:
            logger.warning(
                f"Failed to load task classifications for repository {repository.id}: {e}"
            )
...
"""
    ...
    async def _sync_basic_fields(
        self, repository: Repository, metadata: Dict[str, Any]
    ):
        """同步基础字段"""
    ...
    async def _sync_classifications(
        ...
        """同步分类字段（sphere和task两种分类）"""
        # 同步sphere classifications
        if "classifications" in metadata and isinstance(
            metadata["classifications"], list
        ):
            # 清除现有sphere分类
            ...
            for classification_name in metadata["classifications"]:
                ...
                if classification:
                    await self._add_repository_classification(
                        repository.id, classification.id
                    )

        # 同步task classifications
        if "tasks" in metadata and isinstance(metadata["tasks"], list):
            # 清除现有task分类
            await self._remove_repository_task_classifications(repository.id)

            # 添加新task分类
            for task_name in metadata["tasks"]:
                task_classification = await self._find_task_classification_by_name(
                    task_name
                )
                if task_classification:
                    await self._add_repository_task_classification(
                        repository.id, task_classification.id
                    )
    ...
    async def _find_classification_by_name(self, name: str) -> Optional[Classification]:
        ...
        query = select(Classification).where(
            and_(Classification.name == name, Classification.is_active == True)
        )
    ...
    async def _add_repository_classification(
    ...
    async def _get_repository_task_classifications(
    ...
    async def _find_task_classification_by_name(self, name: str):
        """根据名称查找task分类"""
        from app.models.task_classification import TaskClassification

        query = select(TaskClassification).where(
            and_(TaskClassification.name == name, TaskClassification.is_active == True)
        )

        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def _remove_repository_task_classifications(self, repository_id: int):
        """移除仓库的所有task分类关联"""
        query = select(RepositoryTaskClassification).where(
            RepositoryTaskClassification.repository_id == repository_id
        )

        result = await self.db.execute(query)
        task_classifications = result.scalars().all()

        for task_classification in task_classifications:
            await self.db.delete(task_classification)

    async def _add_repository_task_classification(
        ...
        # 检查task分类是否存在
        task_classification_query = select(TaskClassification).where(
            TaskClassification.id == task_classification_id
        )
        task_classification_result = await self.db.execute(task_classification_query)
        task_classification = task_classification_result.scalar_one_or_none()

        if not task_classification:
            return

        # 创建关联记录
        repo_task_classification = RepositoryTaskClassification(
            repository_id=repository_id, task_classification_id=task_classification_id
        )

        self.db.add(repo_task_classification)
