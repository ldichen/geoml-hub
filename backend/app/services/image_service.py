...
class ImageManagementService:
    ...
    async def upload_image_from_tar(self, db: AsyncSession, repository_id: int, 
                                  tar_file: UploadFile, image_name: str, 
                                  tag: str = "latest", description: str = "",
        ...
        try:
            # 1. 验证仓库和权限
            repository = await self._get_repository_with_validation(db, repository_id, user_id)
            ...
            upload_task = asyncio.create_task(
                self._upload_to_harbor_async(db, image_record.id, tar_file, project_name, 
                                           harbor_repository, tag)
            )
    ...
    async def _upload_to_harbor_async(self, db: AsyncSession, image_id: int, 
                                    tar_file: UploadFile, project_name: str, 
                                    repository_name: str, tag: str):
        """异步上传镜像到Harbor"""
        ...
        try:
            ...
            # 记录开始上传
            await self._add_build_log(db, image_id, "upload", "开始上传镜像到Harbor", "info")
            ...
            try:
                ...
                # 记录错误日志
                await self._add_build_log(db, image_id, "upload", f"镜像上传失败: {error_msg}", "error")
    ...
    async def create_service_with_image(self, db: AsyncSession, repository_id: int,
                                      image_id: int, service_config: Dict, user_id: int) -> Dict:
        """
        基于镜像创建模型服务
        ...
        try:
            ...
            if not image.can_create_service:
                raise Exception(f"镜像 {image.name}:{image.tag} 无法创建服务（状态: {image.status}，已有服务: {image.service_count}/2）")
            
            # 3. 选择最优控制器
            controller_info = await mmanager_controller_manager.select_optimal_controller(
            ...
            # 4. 确保镜像在目标控制器上可用
            image_available = await mmanager_controller_manager.ensure_image_available(
                controller_id, full_image_name, self.harbor_auth
            )
            if not image_available:
                raise Exception(f"无法在控制器 {controller_id} 上获取镜像 {full_image_name}")
            ...
            # 6. 在mManager控制器上创建容器
            create_result = await mmanager_controller_manager.create_container(
            ...
            # 8. 启动容器
            start_result = await mmanager_controller_manager.start_container(
    ...
    async def _get_repository_with_validation(self, db: AsyncSession, repository_id: int, 
                                            user_id: int) -> Repository:
        """获取仓库并验证权限"""
        ...
        if not repository:
            raise Exception(f"仓库 {repository_id} 不存在")
            
        if repository.owner_id != user_id:
            raise Exception("无权限访问此仓库")
            
        return repository
    
    async def _get_image_with_validation(self, db: AsyncSession, image_id: int, 
                                       user_id: int) -> Image:
        """获取镜像并验证权限"""
        result = await db.execute(
            select(Image)
    ...
    async def _add_build_log(self, db: AsyncSession, image_id: int, stage: str, 
    ...
    async def list_repository_images(self, db: AsyncSession, repository_id: int, 
                                   user_id: int) -> List[Dict]:
        """列出仓库的所有镜像"""
        await self._get_repository_with_validation(db, repository_id, user_id)
        
        result = await db.execute(
            select(Image)
            .where(and_(Image.repository_id == repository_id, Image.status != 'deleted'))
        ...
        return [
            {
                ...
                "service_count": img.service_count,
                "can_create_service": img.can_create_service,
    ...
    async def get_image_services(self, db: AsyncSession, image_id: int, 
                               user_id: int) -> List[Dict]:
        """获取镜像的所有服务"""
        ...
        result = await db.execute(
            select(ModelService)
            .where(and_(ModelService.image_id == image_id, ModelService.status != 'deleted'))
