...
class ModelService(Base):
    ...
    model_id = Column(String(255), nullable=False, comment="模型标识符")
    model_ip = Column(String(255), nullable=False, comment="模型服务器IP地址")
    description = Column(Text, comment="服务描述")
    
    # Docker容器相关字段
    docker_image = Column(String(500), comment="Docker镜像名称")
    ...
    gradio_port = Column(Integer, comment="Gradio端口")
    ...
    __table_args__ = (
        ...
        Index('idx_model_services_model_id', 'model_id'),
    )
...
class ServiceInstance(Base):
    ...
    # 容器信息
    container_id = Column(String(255), nullable=False, unique=True, comment="容器ID")
    container_name = Column(String(255), comment="容器名称")
    image_name = Column(String(255), comment="镜像名称")
    
    # 网络配置
    host_port = Column(Integer, comment="主机端口")
    container_port = Column(Integer, default=7860, comment="容器端口")
    
    # 实例状态
    status = Column(String(50), default="created", comment="实例状态: created, starting, running, stopping, stopped, error")
    ...
    __table_args__ = (
        ...
        Index('idx_service_instances_container', 'container_id'),
    )
    
    # 关系
    service = relationship("ModelService", back_populates="instances")
