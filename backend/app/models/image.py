...
class Image(Base):
    ...
    services = relationship("ModelService", back_populates="image", cascade="all, delete-orphan")
    
    # 索引和约束
    __table_args__ = (
        # 唯一约束：同一仓库中original_name:original_tag必须唯一
        UniqueConstraint('original_name', 'original_tag', 'repository_id', 
    ...
    def full_docker_image_name(self):
        ...
        return f"{harbor_host}/{harbor_project}/{self.harbor_repository_path}:latest"
