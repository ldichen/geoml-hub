<script>
	...
	async function commitChanges() {
		...
		try {
			...
			const errorMessage =
				err.response?.data?.detail || err.detail || err.message || '保存文件失败';
	...
	function cancelEdit() {
		if (isModified) {
			...
			pendingNavigation = () => goto(`${base}/${username}/${repositoryName}/blob/${filePath}`);
		} else {
	...
	function showHistory() {
		goto(`${base}/${username}/${repositoryName}/commits/${filePath}`);
	}
	...
	async function saveFileName() {
		...
		try {
			...
			if (newPath !== filePath) {
				...
				await goto(`${base}/${username}/${repositoryName}/edit/${newPath}`, {
					replaceState: true
				});
			}
		} catch (err) {
...
<div class="file-edit-page h-screen flex flex-col">
	...
	<div class="bg-white border-b border-gray-200">
		...
		{:else}
			...
			<div class="container px-4 py-2 flex items-center justify-between">
				<div class="flex items-center space-x-4">
					<div class="text-sm text-gray-600">编辑模式</div>
				...
				<div class="flex items-center space-x-2">
					<Button variant="outline" size="sm" on:click={cancelEdit}>返回</Button>
					<Button
						variant="primary"
						size="sm"
						disabled={!isModified || isSaving}
	...
	<!-- 编辑器主体 -->
	<div
		class="container border-r border-l border-b rounded-lg flex-1 mb-4 overflow-hidden"
...
<!-- 提交模态框 -->
{#if showCommitModal}
	<Modal show={true} title="提交更改" on:close={() => (showCommitModal = false)}>
		<div class="space-y-4">
			<div>
		...
		<div slot="footer" class="flex justify-end space-x-3">
			<Button variant="outline" on:click={() => (showCommitModal = false)}>取消</Button>
			<Button
				variant="primary"
...
{#if showUnsavedChangesModal}
	<Modal show={true} title="未保存的更改" on:close={() => (showUnsavedChangesModal = false)}>
		<div class="space-y-4">
			<p class="text-gray-700">您有未保存的更改。要保存更改还是丢弃它们？</p>
		</div>

		<div slot="footer" class="flex justify-end space-x-3">
			<Button variant="outline" on:click={() => handleUnsavedChanges('cancel')}>取消</Button>
			<Button variant="secondary" on:click={() => handleUnsavedChanges('discard')}>丢弃更改</Button>
			<Button variant="primary" on:click={() => handleUnsavedChanges('save')}>保存更改</Button>
		</div>
	</Modal>
{/if}
