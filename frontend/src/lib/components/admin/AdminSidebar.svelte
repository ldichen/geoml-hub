<script>
    import { page } from '$app/stores';

    export let items = [
        { path: '/admin/dashboard', label: '仪表板', icon: 'dashboard' },
        { path: '/admin/users', label: '用户管理', icon: 'users' },
        { path: '/admin/repositories', label: '仓库管理', icon: 'repositories' },
        { path: '/admin/storage', label: '存储管理', icon: 'storage' },
        { path: '/admin/system', label: '系统监控', icon: 'system' },
        { path: '/admin/settings', label: '系统设置', icon: 'settings' }
    ];

    function getIcon(iconName) {
        const icons = {
            dashboard: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z M8 5a2 2 0 012-2h4a2 2 0 012 2v6H8V5z',
            users: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z',
            repositories: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10',
            storage: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4',
            system: 'M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z',
            settings: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z'
        };
        return icons[iconName] || icons.dashboard;
    }
</script>

<nav class="w-64 bg-white shadow-sm h-full">
    <div class="p-4">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">管理面板</h2>
        <ul class="space-y-2">
            {#each items as item}
                <li>
                    <a
                        href={item.path}
                        class="flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors duration-200 {$page.url.pathname === item.path ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-500' : ''}"
                    >
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getIcon(item.icon)}></path>
                        </svg>
                        {item.label}
                    </a>
                </li>
            {/each}
        </ul>
    </div>
</nav>