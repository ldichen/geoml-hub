<script lang="ts">
  ...
  import { File, Folder, Download, Trash2, Calendar, Search, Grid, List, ChevronRight, ArrowLeft } from 'lucide-svelte';
...
<div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
  ...
  <div class="p-4">
    {#if viewMode === 'list'}
      ...
      <div class="space-y-1">
        <!-- Header -->
        <div class="flex items-center px-3 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
          <div class="flex-1">
            <button
        ...
        <!-- Back Button -->
        {#if currentPath}
          <button class="flex items-center px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md cursor-pointer w-full text-left"
               on:click={navigateUp}>
        ...
        {#each fileStructure.files as file}
          <div class="flex items-center px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md">
    ...
    {:else}
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        <!-- Back Button -->
        {#if currentPath}
          <button class="flex flex-col items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer"
               on:click={navigateUp}>
        ...
        {#each fileStructure.folders as folder}
          <button class="flex flex-col items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer"
            ...
            <span class="text-xs text-gray-900 dark:text-white text-center truncate w-full" title={folder}>
              {folder}
            </span>
          </button>
        {/each}
        ...
        <!-- Files -->
        {#each fileStructure.files as file}
          <div class="flex flex-col items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 group">
            <File class="h-8 w-8 mb-2 {getFileIcon(file.filename)}" />
            <span class="text-xs text-gray-900 dark:text-white text-center truncate w-full" title={file.filename}>
              {file.filename}
            </span>
